chrome.storage.local.get(['saveCorrectAnswers'], function(result) {
    // Default to true if the setting is not defined or is explicitly true
    const saveCorrectAnswers = result.saveCorrectAnswers !== false; 

    if (saveCorrectAnswers) {
        // This script is only injected on quiz pages, so we can always try to load if the feature is enabled.
        loadQuiz();
    } else {
        console.log('[Canvas Hack] Quiz Loader is disabled because "Save Correct Answers" is off.');
    }
});
